...
Options:
  ...
  -t, --track=NAME         practice track: dsa, read, write, or backend
      ...
      --verbose            full output (default is specialty only)
...
func runUnified(root string, opts dailyOptions) int {
	...
	args := moduleArgs(opts)

	switch opts.track {
	case trackDSA:
		if code := runModule(root, "study_code", args, opts.run); code != 0 {
			return code
		}
		if code := runModule(root, "study_play", args, opts.run); code != 0 {
			return code
		}
	case trackRead:
		if code := runModule(root, "study_code", args, opts.run); code != 0 {
	...
	case trackBackend:
		if code := runModule(root, "study_backend", args, opts.run); code != 0 {
			return code
		}
	}
	return 0
