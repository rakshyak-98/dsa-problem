...
func printBlock(b block) {
	...
	fmt.Printf("  STAR:  study_backend/docs/STAR_STORIES.md\n")
	fmt.Println()
}
...
func printCatalog() {
	...
	fmt.Println("Cram plan:    study_backend/docs/INTERVIEW_CRAM_PLAN.md")
}

func main() {
	micro, run, catalog, cram, setup := parseBackendArgs(os.Args[1:])
	if code := runBackend(micro, run, catalog, cram, setup); code != 0 {
		os.Exit(code)
	}
}
