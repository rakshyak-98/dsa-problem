...
```bash
cd study_backend && go run . -- --run
go run -C study_backend/practice/explain/blocks/01_rest_api_jwt .
go run -C study_backend/practice/explain/blocks/02_databases_sql .
```
...
```bash
cd study_backend && go run . -- --micro --run
```
