...
var cues = []patternCue{
	...
	// 01 — sorting & sieve algorithms (folded into the arrays drill)
	{"mergeSort", "01_arrays_reflex", levelRecall,
		"return a new ascending copy using a stable divide-and-conquer sort",
		"sort with a guaranteed O(n log n) bound, stability matters, or 'count while sorting'",
		"split in half, recurse both sides, merge with two pointers taking the smaller (<= keeps it stable)",
		"count smaller numbers after self — tally cross-pairs during the merge step"},
	{"quickSort", "01_arrays_reflex", levelRecall,
		"return an ascending copy by partitioning in place around a pivot",
		"in-place O(n log n) average sort, no second array allowed",
		"Lomuto: pivot last, i marks the < frontier, swap pivot home, recurse lo..i-1 and i+1..hi",
		"3-way partition so runs of equal keys don't degrade to O(n^2)"},
	{"sieve", "01_arrays_reflex", levelRecall,
		"return every prime number less than or equal to n",
		"'all primes up to n', or many primality checks over one fixed range",
		"cross out multiples of each p starting at p*p; the entries left standing are prime",
		"count primes below n, or the segmented sieve for a high range"},

	// 02 — the no-hash counterpart (folded into the hashing drill)
	{"singleNumber", "02_hashing_reflex", levelRecall,
		"every value appears twice except one — return the one that doesn't",
		"exactly one unpaired element and O(1) extra space is demanded",
		"XOR the whole array; equal values cancel to 0 and the loner survives",
		"the value that appears once while the rest appear three times (bit counts mod 3)"},

	// 03 — one-pass partition (folded into the two-pointer drill)
	{"dutchFlag", "03_two_pointers_reflex", levelPattern,
		"sort an array whose values are only 0, 1, 2 in one pass and constant space",
		"only a handful of distinct values, sort in place in a single sweep",
		"three pointers low/mid/high; on 0 swap to low, on 2 swap to high and hold mid, on 1 step mid",
		"partition an array into <pivot, ==pivot, >pivot regions"},

	// 04 — divide-and-conquer / logarithmic reduction (folded into the binary search drill)
	{"quickSelect", "04_binary_search_reflex", levelPattern,
		"return the kth smallest value (1-indexed) without fully sorting the array",
		"'kth largest / smallest' with an O(n) average requirement",
		"partition; pivot index == k-1 is the answer, otherwise recurse into the one side that holds rank k",
		"median of an unsorted array, or the k closest points to the origin"},
	{"fastPow", "04_binary_search_reflex", levelPattern,
		"return x raised to the integer n (n may be negative) in O(log n)",
		"exponent is large, or 'compute x^n without the library power call'",
		"flip negative n to 1/x; fold n bit by bit — square x each step, multiply it in when the low bit is set",
		"modular exponentiation x^n mod m, or matrix power for a linear recurrence"},
	{"gcd", "04_binary_search_reflex", levelRecall,
		"return the greatest common divisor of a and b",
		"reduce a fraction, tile a length evenly, 'largest step landing on both marks'",
		"Euclid: while b != 0 { a, b = b, a % b }; return a",
		"lcm(a,b) = a / gcd(a,b) * b, or fold gcd across a whole array"},

	// 07 — adjacency-list algorithms (folded into the graphs drill)
	{"dfs", "07_graphs_reflex", levelRecall,
		"return the reachable nodes from start in recursive preorder",
		"explore a graph as deep as one branch goes before backtracking",
		"mark on entry, append, recurse into unseen neighbours in order (stack = same, push reversed)",
		"count connected components — one traversal per unvisited node"},
	{"bfs", "07_graphs_reflex", levelRecall,
		"return the reachable nodes from start in breadth order",
		"explore a graph one ring of neighbours at a time from a source",
		"queue; mark a node on enqueue, never on dequeue, or it lands in the queue twice",
		"multi-source spread — seed the queue with every start at distance 0"},
	{"bfsShortestPath", "07_graphs_reflex", levelPattern,
		"fewest edges from src to dst; 0 if equal, -1 if unreachable",
		"'minimum steps' / 'shortest path' on an unweighted graph",
		"level-by-level BFS; the count of levels crossed is the edge count — never DFS for this",
		"word ladder — the graph is implicit, neighbours are one-letter edits"},
	{"topoSort", "07_graphs_reflex", levelPattern,
		"a linear order of a directed graph, or nil when a cycle makes it impossible",
		"'ordering' / 'prerequisites' / 'is there a cycle' on a directed graph",
		"Kahn: queue the zero-in-degree nodes, pop one, decrement its targets; leftovers = cycle",
		"course schedule with the order returned, or alien-dictionary letter order"},
	{"dijkstra", "07_graphs_reflex", levelTransfer,
		"shortest distance from src to every node over non-negative weighted edges",
		"cheapest total-weight path, edge weights vary and are non-negative",
		"min-heap of (node, dist); pop the closest, relax its edges, skip stale entries",
		"network delay time, or cheapest flights within k stops (add a hop budget to the state)"},
}
