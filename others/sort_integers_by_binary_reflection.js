...
var sortByReflection = function (nums) {
  let result = [];
  const reflectedList = nums.map((num) => {
    const binary = num.toString(2);
    let reverse = binary.split().reverse().join();
    let reflectedValue = parseInt(reverse, 2);
    return {
      original: num,
  ...
  reflectedList.sort((a, b) => {
    if(a.reflected != b.reflected){
      return a.reflected - b.reflected;
    }
    return a.original - b.original;
  })
  return reflectedList.map((i) => i.original);
};
