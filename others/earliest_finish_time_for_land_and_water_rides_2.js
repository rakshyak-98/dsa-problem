...
var earliestFinishTime = function(landStartTime, landDuration, waterStartTime, waterDuration) {
  ...
  let earliestTotalFinish = Infinity;

  for (let j = 0; j < m; j++) {
    const finishTime = Math.max(minLandFinish, waterStartTime[j]) + waterDuration[j];
    if (finishTime < earliestTotalFinish) {
      earliestTotalFinish = finishTime;
    }
  }

  for (let i = 0; i < n; i++) {
    const finishTime = Math.max(minWaterFinish, landStartTime[i]) + landDuration[i];
    if (finishTime < earliestTotalFinish) {
      earliestTotalFinish = finishTime;
    }
  }
...
console.log(earliestFinishTime([3], [6], [4,1], [2,8], ))
// Input: landStartTime = , landDuration = , waterStartTime = , waterDuration = 