...
5. **Daily practice:** write `drills/write/reflex/10_math_reflex` (`go run . -- --run-math` in `study_play`); read `drills/read/weekday/08_math_concepts` (`go run . -- --run-math` in `study_code`).
